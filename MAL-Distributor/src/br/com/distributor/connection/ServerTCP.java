...
public class ServerTCP implements Runnable{
    ...
    private final ControllerDistributor ctrl; //Variavel que recebe objeto que contem as infomações do sistema
    
    //Construtor
    public ServerTCP(int porta, ControllerDistributor ctrl) throws IOException {
    ...
    public void run() {
        try{
            while(!servidor.isClosed()){//Laço de repetição para a criação de várias threads a medida que receber novas conexões
                ...
                //new ActionsDistributor(servidor.accept(),ctrl).start();
